"""Grow Tower — rename + label the 4 named-color `text` entities (2026-07-16).

Background:
  Replaced the per-color hardcoded RGB literals with a reusable named-color
  palette: 4 single-line `text` entities ("R,G,B" 0-255), read by every LED
  script (aesthetic idle, MEDIUM alert, HIGH full-flash) via the shared
  `led_set_color_from_text` script. On reflash they landed with the usual
  `front_sunroom_` area-prefix quirk:

    text.front_sunroom_grow_tower_color_yellow
    text.front_sunroom_grow_tower_color_orange
    text.front_sunroom_grow_tower_color_green
    text.front_sunroom_grow_tower_color_blue

This script (idempotent, dry-run by default):
  1. Renames the 4 entity_ids, stripping the `front_sunroom_` prefix.
  2. Applies the same tagging-standard labels used for the other LED-tuning
     ...
     tuning value, same carve-out as the number/select LED-tuning entities).

Run `python registry_export.py --label pre-grow-tower-color-text` first for a snapshot.
...
RENAMES: list[tuple[str, str]] = [
    ("text.front_sunroom_grow_tower_color_yellow", "text.grow_tower_color_yellow"),
    ("text.front_sunroom_grow_tower_color_orange", "text.grow_tower_color_orange"),
    ("text.front_sunroom_grow_tower_color_green", "text.grow_tower_color_green"),
    ("text.front_sunroom_grow_tower_color_blue", "text.grow_tower_color_blue"),
]
